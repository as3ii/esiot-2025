...
#define FRAME_SYNC_DELAY                                                       \
  ((FRAME_SYNC_PERIOD - (NBR_CHANNELS * DEFAULT_PULSE_WIDTH)) / 128)
// number of microseconds of calculation overhead to be subtracted from
// pulse timings
#define DELAY_ADJUST 8

static servo_t
  servos[NBR_CHANNELS + 1]; // static array holding servo data for all channels

static volatile uint8_t Channel;  // counter holding the channel being pulsed
static volatile uint8_t ISRCount; // iteration counter used in the ISR
uint8_t ChannelCount = 0; // counter holding the number of attached channels
static boolean isStarted = false; // indicates if the ISR has been initialised
...
ISR(TIMER2_OVF_vect) {
  ...
  if (ISRCount == servos[Channel].counter) {
    ...
    if ((Channel != FRAME_SYNC_INDEX) &&
        (Channel <= NBR_CHANNELS)) { // check if we need to pulse this channel
      if (servos[Channel].Pin.isActive) { // check if activated
...
uint8_t ServoTimer2::attach(int pin) const {
  if (!isStarted) {
    initISR();
  }
  if (this->chanIndex > 0) {
...
void ServoTimer2::detach() const {
  servos[this->chanIndex].Pin.isActive = false;
}

void ServoTimer2::write(int pulsewidth) const {
  writeChan(
    this->chanIndex,
    pulsewidth); // call the static function to store the data for this servo
}

int ServoTimer2::read() const {
  int pulsewidth = 0;
  if (this->chanIndex > 0) {
    pulsewidth = (servos[this->chanIndex].counter * 128) +
                 ((255 - servos[this->chanIndex].remainder) / 2) + DELAY_ADJUST;
  }
  return pulsewidth;
}

boolean ServoTimer2::attached() const {
...
static void initISR() {
  ...
  isStarted = true; // indicates this initialisation code has been executed
}
